...
exports.convertDate = function(usDate) {
    var dateParts = usDate.split(/(\d{2})[\/ -](\d{2})[\/ -](\d{4})/);
    return dateParts[3] + "-" + dateParts[2] + "-" + dateParts[1];
}
exports.convertOriginalDate = function(usDate) {
    var dateParts = usDate.split(/(\d{4})[\/ -](\d{2})[\/ -](\d{2})/);
    return dateParts[3] + "/" + dateParts[2] + "/" + dateParts[1];
}
exports.validateOriginalDate = function (d) {
    return /^(0?[1-9]|[12][0-9]|3[01])\/(0?[1-9]|1[012])\/\d{4}$/.test(d); //MM-dd-yyyy
};
